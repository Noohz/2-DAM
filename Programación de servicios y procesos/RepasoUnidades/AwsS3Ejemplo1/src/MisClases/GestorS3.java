...
public class GestorS3 {
	...
	// Constructor
	GestorS3() {
		// Configuramos el cliente de S3 indic�ndole las credenciales de conexi�n
		/*
		 * Si tuvi�ramos una cuenta AWS convencional deber�amos realizar la conexi�n
		 * bas�ndonos en las credenciales IAM del usuario, indicando su "ACCESS_KEY" y
		 * "SECRET_KEY". Si accedemos a S3 a trav�s del Learner Lab, necesitamos una
		 * credencial basada en el token de sesi�n que proporciona acceso temporal a los
		 * servicios de AWS. En este caso hay que especificar la "ACCESS_KEY",
		 * "SECRET_KEY" y "SESSION_TOKEN que se pueden ver en AWS Details tras lanzar el
		 * laboratorio virtual. Tambi�n existe la posibilidad de dar estos datos en un
		 * fichero del proyecto para no tener que indicarlos en el c�digo
		 */

		clienteS3 = S3Client.builder().region(Region.US_EAST_1) // Indicamos la regi�n donde trabajaremos
...
//            .credentialsProvider(StaticCredentialsProvider.create(AwsSessionCredentials.create("ACCESS_KEY", "SECRET_KEY", "SESSION_TOKEN")))
				...
				.build(); // El m�todo build() es el que me devuelve el cliente de sesi�n.
	...
	void listarContenidoBucket() {
		...
		// Le pedimos al usuario el prefijo (carpeta) de los contenidos que queremos
		// listar
		System.out.print("Introduce el prefijo(carpeta) de los contenidos. P ej: 'datos/' : ");
		String prefijo = Aplicacion.teclado.nextLine();

		try {
			// Llamamos a la API de S3 para listar objetos en el bucket que tengan en su key
			// el prefijo indicado
			ListObjectsResponse listObjectsResponse = clienteS3
					.listObjects(builder -> builder.bucket(nombreBucket).prefix(prefijo));

			// Vamos recorriendo la lista de objetos que hay en los contenidos de la
			// respuesta
			for (S3Object s3Object : listObjectsResponse.contents()) {
				// Imprime informaci�n sobre cada objeto
				System.out.println("Nombre del objeto: " + s3Object.key());
				System.out.println("Tama�o del objeto: " + s3Object.size());
				System.out.println("�ltima modificaci�n: " + s3Object.lastModified());
	...
	void crearBucket() {
		...
		try {
			...
			System.out.println("El bucket " + nuevoNombre + " se ha creado con �xito.");
		} catch (S3Exception e) {
			// Ha habido problemas en la creaci�n del bucket
			System.out.println("Ha habido un problema en la creaci�n del bucket. Quiz�s ya exista uno con ese nombre");
		}
	}
	...
	void borrarContenidoBucket() {
		...
		// Este m�todo borra todo el contenido que haya en la carpeta indicada del
		// bucket de trabajo

		// Ruta donde borraremos el contenido
		String ruta = "datosGenerados/";

		// Hemos de recordar que en S3 no existe el concepto de carpeta, sino que hay
		// objetos con barras en sus nombres de ficheros.
		// Lo que se puede hacer es borrar todos los objetos que tengan el mismo
		// prefijo, pero primero hay que ir busc�ndolos con list_objects

		// Lista todos los objetos que tengan el prefijo con el nombre de la carpeta
		ListObjectsResponse listObjectsResponse = clienteS3
				.listObjects(builder -> builder.bucket(nombreBucket).prefix(ruta));

		// Vamos recorriendo todos los objetos de la lista de respuesta obtenida y
		// eliminamos cada objeto uno a uno
		for (S3Object s3Object : listObjectsResponse.contents()) {

			DeleteObjectRequest deleteObjectRequest = DeleteObjectRequest.builder().bucket(nombreBucket)
	...
	void leerContenidoObjetoTexto() {
		...
		try {
			// Llamamos a la API de S3 para obtener informaci�n sobre el objeto
			...
			// Leemod l�nea por l�nea y muestra el contenido
			String linea;
	...
	void generarContenidoFicherosTexto() {
		...
		// Creamos 10 ficheros de 1000 n�meros aleatorios cada 5 segundos y los
		// guardamos una carpeta con el momento y un fichero llamado datos.dat
		...
		for (int contador = 0; contador < 10; contador++) {
			...
			// Generamos un fichero con 1000 n�meros aleatorios
			...
			// Una vez que tengo los 1000 n�meros escritos en el ByteOutputStream
			...
			// Establecemos el nombre que tendr� el objeto
			String nombreObjeto = ruta + cadenaFechaHora + "/contenido.dat";

			// Creamos la solicitud para sobrescribir el objeto
			PutObjectRequest putObjectRequest = PutObjectRequest.builder().bucket(nombreBucket).key(nombreObjeto)
					.build();
...
//        String endpoint = "https://fjgcdam2024prueba1.s3.us-east-1.amazonaws.com/datos2/Titanic.csv";
		...
		try {
			...
			// Abre una conexi�n HTTP
			HttpURLConnection connection = (HttpURLConnection) url.openConnection();

			// Configura la solicitud HTTP GET
			connection.setRequestMethod("GET");
			// A�ade las cabeceras necesarias para la autenticaci�n
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				in.close();
				System.out.println("Error en la solicitud. C�digo de respuesta: " + responseCode);
			}
